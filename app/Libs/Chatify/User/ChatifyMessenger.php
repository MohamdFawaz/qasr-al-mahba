...
namespace App\Libs\Chatify\User;
...
class ChatifyMessenger
{
    ...
    {
        return app()->make(ChatifyMessenger::class)->pusher->trigger($channel, $event, $data);
    }
    ...
    public static function pusherAuth($channelName, $socket_id, $data = null){
        return app()->make(ChatifyMessenger::class)->pusher->socket_auth($channelName, $socket_id, $data);
    }
    ...
    public static function fetchMessage($id){
        ...
        return [
            ...
            'viewType' => ($msg->from_id == Auth::guard('users')->user()->id) ? 'sender' : 'default',
            'seen' => $msg->seen,
        ];
    }
    ...
    public static function messageCard($data, $viewType = null){
        ...
        return view('Chatify::user.layouts.messageCard',$data)->render();
    }
    ...
    public static function fetchMessagesQuery($user_id){
        return Message::where('from_id',Auth::guard('users')->user()->id)->where('to_id',$user_id)
                    ->orWhere('from_id',$user_id)->where('to_id',Auth::guard('users')->user()->id);
    }
    ...
    public static function makeSeen($user_id){
        Message::Where('from_id',$user_id)
                ->where('to_id',Auth::guard('users')->user()->id)
                ->where('seen',0)
                ->update(['seen' => 1]);
        return 1;
    }
    ...
    public function countUnseenMessages($user_id){
        return Message::where('from_id',$user_id)->where('to_id',Auth::guard('users')->user()->id)->where('seen',0)->count();
    }
    ...
    public static function getContactItem($messenger_id, $user){
        // get last message
        $lastMessage = app()->make(ChatifyMessenger::class)->getLastMessageQuery($user->id);

        // Get Unseen messages counter
        $unseenCounter = app()->make(ChatifyMessenger::class)->countUnseenMessages($user->id);

        return view('Chatify::user.layouts.listItem', [
            'get' => 'users',
            'user' => $user,
    ...
    public static function inFavorite($user_id){
        return Favorite::where('user_id', Auth::guard('users')->user()->id)
    ...
    public function makeInFavorite($user_id, $action){
        if ($action > 0) {
            ...
            $star->user_id = Auth::guard('users')->user()->id;
            $star->favorite_id = $user_id;
            $star->save();
            return $star ? true : false;
        }else{
            // UnStar
            $star = Favorite::where('user_id',Auth::guard('users')->user()->id)->where('favorite_id',$user_id)->delete();
            return $star ? true : false;
        }
    }
